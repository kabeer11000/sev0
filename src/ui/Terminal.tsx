      ...
      <div ref={scrollRef} className="min-h-0 flex-1 overflow-y-auto px-4 py-3">
        {lines.length === 0 ? (
          <div className="flex flex-col gap-2 py-6 text-center">
            <div className="mx-auto text-[11px]" style={{ color: 'var(--fg-faint)' }}>
              connected to{' '}
              <span className="rounded-full px-2 py-0.5" style={{ background: 'rgba(238, 90, 54, 0.16)', color: 'var(--accent)' }}>
                {nodeId}
              </span>
            </div>
            <div className="mx-auto max-w-[36ch] text-[11px] leading-relaxed" style={{ color: 'var(--fg-faint)' }}>
              type <span style={{ color: 'var(--accent)' }}>help</span> for available commands —{' '}
              {helpFor(kind).join(', ')}
            </div>
          </div>
        ) : (
            ...
            {lines.map((l, i) => (
              <div key={i} className="mb-2">
                <div style={{ color: 'var(--fg)' }}>
                  <span
                    className="mr-2 inline-block rounded-md px-1.5 py-0.5 text-[10.5px] font-semibold"
                    style={{ background: 'rgba(238, 90, 54, 0.16)', color: 'var(--accent)' }}
                  >
                    {nodeId}
                  </span>
                  {l.cmd}
                ...
                {l.output.map((o, j) => (
                  <div key={j} style={{ color: 'var(--fg-muted)' }} className="whitespace-pre-wrap pl-2">
                    {o}
                  </div>
                ))}
              </div>
        ...
        <div className="mt-1 flex items-center gap-2">
          <span
            ...
            style={{ background: 'rgba(238, 90, 54, 0.20)', color: 'var(--accent)' }}
          >
            {nodeId}
          </span>
